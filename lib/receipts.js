...
// Lazy-require wot.js to avoid circular dependency
// (wot.js requires receipts.js at the bottom, receipts.js requires wot.js)
let _wot = null;
function getWot() {
...
// Convenience accessors (resolved lazily)
function getPublishAttestation() { return getWot().publishAttestation; }
function getQueryRelays() { return getWot().queryRelays; }
function getRELAYS() { return getWot().RELAYS; }
function getNAMESPACE() { return getWot().NAMESPACE; }
...
async function publishReceipt(secretKey, dvmResult, opts = {}) {
  ...
  const { event, results } = await getPublishAttestation()(
    secretKey,
    targetPubkey,
    type,
    comment,
...
async function queryDVMHistory(myPubkey, opts = {}) {
  ...
  const _queryRelays = getQueryRelays();
  const requests = await _queryRelays(requestFilter, relays);
  ...
  const results = await _queryRelays(resultFilter, relays);
  ...
  if (dvmPubkeys.length > 0) {
    ...
    const atts = await _queryRelays(attFilter, relays);
...
async function publishBatchAttestations(secretKey, targets, opts = {}) {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      const { event, results: relayResults } = await getPublishAttestation()(
        secretKey,
        target.pubkey,
