...
const {
  ...
  filterByCategory,
  calculateCategoryScore: computeCategoryScore,
  calculateAllCategoryScores: computeAllCategoryScores,
  TYPE_MULTIPLIERS,
  VALID_TYPES,
  POSITIVE_TYPES,
  NEGATIVE_TYPES,
...
// ─── Re-attestation & Deduplication (v0.6.0) ────────────────────

/**
 * Publish a re-affirmation attestation of the same type to the same target.
 *
 * Conceptually identical to publishAttestation but defaults to "Reaffirmed: " prefix
 * and optionally references the original attestation event ID via an 'e' tag.
 *
 * @param {Buffer|Uint8Array} secretKey - 32-byte secret key
 * @param {string} targetPubkey - hex pubkey of the agent being attested
 * @param {string} type - attestation type
 * @param {string} comment - human-readable explanation (auto-prefixed with "Reaffirmed: ")
 * @param {object} [opts] - optional: { originalEventId, relays, expiration, eventRef, relayHint }
 * @returns {Promise<{event, results}>}
 */
async function reaffirmAttestation(secretKey, targetPubkey, type, comment, opts = {}) {
  // Prefix comment with "Reaffirmed: " unless it already starts with it
  const prefixedComment = comment && comment.startsWith('Reaffirmed: ')
    ? comment
    : 'Reaffirmed: ' + (comment || '');

  // If originalEventId is provided, use it as the eventRef
  const effectiveOpts = { ...opts };
  if (opts.originalEventId) {
    effectiveOpts.eventRef = opts.originalEventId;
  }

  return publishAttestation(secretKey, targetPubkey, type, prefixedComment, effectiveOpts);
}

/**
 * Query all attestations for a target and return only the most recent
 * from each (attester, type) pair.
 *
 * @param {string} targetPubkey - hex pubkey to look up
 * @param {object} [opts] - optional: { relays, type, limit, includeRevoked }
 * @returns {Promise<Array>} - deduplicated attestation events
 */
async function queryLatestAttestations(targetPubkey, opts = {}) {
  const attestations = await queryAttestations(targetPubkey, opts);
  return deduplicateAttestations(attestations);
}
...
/**
 * Publish a work-completed attestation with structured content.
 *
 * Convenience wrapper around publishAttestation that formats the content
 * in the recommended machine-readable format:
 *   "Work completed | <description> | <amount> sats [| <reference>]"
 *
 * @param {Buffer|Uint8Array} secretKey - 32-byte secret key
 * @param {string} providerPubkey - hex pubkey of the agent who completed the work
 * @param {string} description - what was delivered (e.g., "Translation EN→DE")
 * @param {object} [opts] - Options:
 *   - amountSats: Amount paid in sats
 *   - reference: Invoice hash, event ID, or other reference
 *   - eventRef: Nostr event ID to reference via e-tag
 *   - relayHint: Relay hint for the e-tag
 *   - relays: Relay URLs to publish to
 *   - expiration: Custom expiration timestamp (false to disable)
 * @returns {Promise<{event, results}>}
 */
async function publishWorkCompleted(secretKey, providerPubkey, description, opts = {}) {
  const parts = ['Work completed'];
  if (description) parts.push(description);
  if (opts.amountSats) parts.push(`${opts.amountSats} sats`);
  if (opts.reference) parts.push(opts.reference);
  const comment = parts.join(' | ');

  return publishAttestation(secretKey, providerPubkey, 'work-completed', comment, {
...
// ─── Category-Based Trust Scoring (v0.8.0) ──────────────────────

/**
 * Calculate trust score for a specific category.
 *
 * @param {string} pubkey - hex pubkey
 * @param {string} category - category name (commerce, identity, code, general, or attestation type)
 * @param {object} [opts] - { relays, depth, halfLifeDays, _cache }
 * @returns {Promise<object>} Score result with category field
 */
async function calculateCategoryScore(pubkey, category, opts = {}) {
  const depth = opts.depth || 0;
  const cache = opts._cache || new Map();
  const relays = opts.relays || RELAYS;
  const halfLifeDays = opts.halfLifeDays || 90;
  ...
  if (attestations.length === 0) {
    return {
      raw: 0, display: 0, attestationCount: 0,
      positiveCount: 0, negativeCount: 0, gatedCount: 0,
      breakdown: [],
  ...
  return computeCategoryScore(attestations, zapTotals, category, {
    halfLifeDays,
    depth,
    maxDepth: 2,
    cache,
 ...
 * @param {string} pubkey - hex pubkey
 * @param {object} [opts] - { relays, depth, halfLifeDays, _cache }
 * @returns {Promise<object>} Object with category names as keys
 */
async function getAllCategoryScores(pubkey, opts = {}) {
  const depth = opts.depth || 0;
  const cache = opts._cache || new Map();
  const relays = opts.relays || RELAYS;
  const halfLifeDays = opts.halfLifeDays || 90;
  ...
  if (attestations.length === 0) {
    ...
    for (const cat of ALL_CATEGORY_NAMES) {
      results[cat] = { ...empty, category: cat };
    }
    return results;
  }
  ...
  return computeAllCategoryScores(attestations, zapTotals, {
    halfLifeDays,
    depth,
    maxDepth: 2,
    cache,
...
// ─── Trust Path Discovery (v0.8.0) ─────────────────────────────

/**
 * Find a trust path between two agents via BFS through the attestation graph.
 *
 * @param {string} fromPubkey - starting pubkey
 * @param {string} toPubkey - target pubkey
 * @param {object} [opts] - { relays, maxDepth (default 3) }
 * @returns {Promise<{found: boolean, path: Array, hops: number}>}
 */
async function findTrustPath(fromPubkey, toPubkey, opts = {}) {
  const relays = opts.relays || RELAYS;
  const maxDepth = opts.maxDepth !== undefined ? opts.maxDepth : 3;

  if (fromPubkey === toPubkey) {
    return { found: true, path: [{ pubkey: fromPubkey, type: null, score: null }], hops: 0 };
  }

  // BFS
  // Queue entries: { pubkey, depth, pathSoFar }
  const visited = new Set([fromPubkey]);
  const queue = [{ pubkey: fromPubkey, depth: 0, pathSoFar: [{ pubkey: fromPubkey, type: null, score: null }] }];
  ...
  while (queue.length > 0) {
    ...
    // Find all agents that `current.pubkey` has attested (outgoing attestations)
    const outgoing = await queryOutgoingAttestations(current.pubkey, { relays });
    ...
    for (const att of outgoing) {
      ...
      if (visited.has(targetPk)) continue;
      visited.add(targetPk);
      ...
      const attType = lTag ? lTag[1] : 'unknown';

      // Get a quick score for this hop node (no recursion, just use cache)
      let hopScore = null;
      try {
        const s = await calculateTrustScore(targetPk, { relays, _cache: new Map() });
        hopScore = s.display;
      } catch (_) {}

      const newPath = [...current.pathSoFar, { pubkey: targetPk, type: attType, score: hopScore }];

      if (targetPk === toPubkey) {
        return { found: true, path: newPath, hops: newPath.length - 1 };
      }

      queue.push({ pubkey: targetPk, depth: current.depth + 1, pathSoFar: newPath });
    }
  }
...
/**
 * Query outgoing attestations FROM a given pubkey (attestations they've made).
 *
 * @param {string} pubkey - hex pubkey of the attester
 * @param {object} [opts] - { relays, limit }
 * @returns {Promise<Array>} - attestation events
 */
async function queryOutgoingAttestations(pubkey, opts = {}) {
  const filter = {
    kinds: [1985],
    '#L': [NAMESPACE],
    authors: [pubkey],
  ...
  events = events.filter(e => {
    const pTag = e.tags ? e.tags.find(t => t[0] === 'p') : null;
    return pTag && pTag[1] !== pubkey;
  });
...
module.exports = {
  ...
  // Trust Path Discovery (v0.8.0)
  findTrustPath,
  queryOutgoingAttestations,

  // Re-attestation & Deduplication (v0.6.0)
  reaffirmAttestation,
  queryLatestAttestations,

  // Revocations
  publishRevocation,
  queryRevocations,

  // Work-Completed (v0.5.0)
  publishWorkCompleted,

  // DVM Receipts (v0.4.0)
  publishReceipt: receipts.publishReceipt,
  parseDVMResult: receipts.parseDVMResult,
  parseDVMFeedback: receipts.parseDVMFeedback,
  queryDVMHistory: receipts.queryDVMHistory,
  watchDVMResults: receipts.watchDVMResults,
  publishBatchAttestations: receipts.publishBatchAttestations,
  generateReceiptCandidate: receipts.generateReceiptCandidate,
  DVM_KIND_NAMES: receipts.DVM_KIND_NAMES,
  ...
  // Candidate-Confirm (v0.7.0)
  CandidateStore: candidates.CandidateStore,
  watchDVMCandidates: candidates.watchDVMCandidates,
  generateL402Candidate: candidates.generateL402Candidate,
  generateCandidate: candidates.generateCandidate,
  filePersistence: candidates.filePersistence,
