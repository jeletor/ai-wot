    ...
    try {
      ...
      switch (route.handler) {
        ...
        case 'candidatesAction':
          result = await handleCandidatesAction(candidateStore, route.candidateId, route.action, req);
          break;
      }
...
async function handleCandidatesAction(store, id, action, req) {
  if (!store) return { status: 404, body: { error: 'Candidate store not configured' } };
  ...
  if (action === 'confirm') {
    ...
    return { status: 200, body: { confirmed: true, candidate: c } };
  }
